/**
 * Squadra — Reset ALL + Seed
 * Supprime TOUTES les données (tous les users) puis re-seed.
 * Usage: node scripts/reset-seed.mjs
 */
...
async function resetAll() {
  ...
  const { data: groupes, error: gFetchErr } = await supabase.from('groupes').select('id')
  if (gFetchErr) { console.error('  ❌ Fetch groupes:', gFetchErr.message); return }

  if (!groupes || groupes.length === 0) {
    console.log('  ℹ️  Aucun groupe existant')
    return
  }

  const groupeIds = groupes.map(g => g.id)
  console.log(`  📋 ${groupes.length} groupes trouvés`)

  // Récupérer tous les joueurs
  const { data: joueurs } = await supabase.from('joueurs').select('id').in('groupe_id', groupeIds)
  const joueurIds = (joueurs || []).map(j => j.id)

  if (joueurIds.length > 0) {
    // Supprimer avis
    const { error: aErr } = await supabase.from('avis').delete().in('joueur_id', joueurIds)
    if (aErr) console.error('  ❌ Avis:', aErr.message)
    else console.log(`  ✅ Avis supprimés`)

    // Supprimer liens_notation si existe
    try {
      await supabase.from('liens_notation').delete().in('joueur_id', joueurIds)
    } catch(e) {}

    // Supprimer joueurs
    const { error: jErr } = await supabase.from('joueurs').delete().in('groupe_id', groupeIds)
    if (jErr) console.error('  ❌ Joueurs:', jErr.message)
    else console.log(`  ✅ ${joueurIds.length} joueurs supprimés`)
  }

  // Supprimer tirages
  const { error: tErr } = await supabase.from('tirages').delete().in('groupe_id', groupeIds)
  if (tErr) console.error('  ❌ Tirages:', tErr.message)
  else console.log(`  ✅ Tirages supprimés`)

  // Supprimer groupes
  const { error: gErr } = await supabase.from('groupes').delete().in('id', groupeIds)
  if (gErr) console.error('  ❌ Groupes:', gErr.message)
  else console.log(`  ✅ ${groupes.length} groupes supprimés`)
}
...
async function seed(userId) {
  ...
  for (const groupeData of GROUPES) {
    console.log(`\n📁 ${groupeData.emoji} ${groupeData.nom} (${groupeData.sport})`)

    const code = Math.random().toString(36).substring(2, 8).toUpperCase()
    const { data: groupe, error: gErr } = await supabase
      .from('groupes')
      .insert({ ...groupeData, user_id: userId, code })
      .select()
      .single()

    if (gErr) { console.error('  ❌', gErr.message); continue }
    console.log(`  ✅ Créé — code: ${code}`)
    ...
    for (let i = 0; i < joueursData.length; i++) {
      ...
      if (jErr) { console.error(`  ❌ ${jd.prenom}:`, jErr.message); continue }
      joueursInserted.push(joueur)
      console.log(`  👤 ${jd.prenom} ${jd.nom} — ${jd.poste} — ★${note}`)

      const avis = AVIS_TEMPLATES[i % AVIS_TEMPLATES.length]
    ...
    if (joueursInserted.length >= 4) {
      ...
      for (let t = 0; t < 3; t++) {
        ...
        const total1 = Math.round(eq1.reduce((s, j) => s + j.note_moyenne, 0) * 10) / 10
        const total2 = Math.round(eq2.reduce((s, j) => s + j.note_moyenne, 0) * 10) / 10
        const equilibre = Math.round((1 - Math.abs(total1 - total2) / Math.max(total1, total2)) * 100)
        ...
        console.log(`  🎯 Match ${t + 1} — ${dates[t]} — équilibre ${equilibre}%`)
      }
    }
  }
}
...
async function main() {
  console.log('🔄 Squadra — Reset All + Seed\n')

  // Auth avec le user actuel ou anonyme
  ...
  console.log(`✅ User actif: ${user.id}\n`)

  await resetAll()
  await seed(user.id)

  console.log('\n🎉 Terminé ! Relance l\'app et vide le cache (R majuscule).')
}
